...
% select path in python/output/robustness
cd(uigetdir())
...
for o=3:size(global_folders,1)
    ...
    for l=3:size(model_folders,1)
        ...
        for s=3:size(split_folders,1)
            ...
            clearvars -except s split_path split_folders l model_path model_folders o global_path global_folders Targets_Subject_Split Feature_EMG_Split Feature_Splits ...
            ...
            for i=1:size(Outputs.R_pred_act_epsilon,1)
                ...
                clearvars -except i s split_path split_folders Outputs split Feature_Splits_Temp l model_path model_folders o global_path global_folders Targets_Subject_Split Feature_EMG_Split Feature_Splits ...
                    RpredAct_Output RpredAct_flat_Output RpredAct_zb_Output RpredDom_Output RpredDom_flat_Output RpredDom_zb_Output Ypred_Output Yact_Output Feature_Output
        ...
        % only the results of the corresponding individual are stored, all others are withdrawn to save storage
        Feature_Output_Temp{1} = Feature_Output{1};
        RpredAct_Output_Temp{1} = RpredAct_Output{1};
        Ypred_Output_Temp{1} = Ypred_Output{1};
        Yact_Output_Temp{1} = Yact_Output{1};
        
        clearvars Feature_Output RpredAct_Output Ypred_Output Yact_Output
        
        Feature_Output = Feature_Output_Temp;
        RpredAct_Output = RpredAct_Output_Temp;
        Ypred_Output = Ypred_Output_Temp;
        Yact_Output = Yact_Output_Temp;
